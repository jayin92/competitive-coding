...
pii xx;
vector<pii> blockx;
priority_queue<ans_, vector<ans_>, cmp> ans;
...
void solve(pii x, vector<pii> block, int r){
    debug(x, r);
    ...
    for(int i=7-r, cnt = 0;cnt<4;cnt++, i--){
        debug(r + i, cnt, i);
        res = rotate(x, block, i % 4);
        if(res.X.X != -1){
           solve(res.X, res.Y, (r + i) % 4);
        }
    }
